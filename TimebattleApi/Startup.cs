...
namespace TimebattleApi
{
	public class Startup
	{
		...
		public void ConfigureServices(IServiceCollection services)
		{
			...
			services.AddSingleton<IConnectionIdHolder, SectorInMemoryConnectionIdHolder>();
			services.AddHostedService<SectorMessagesBackgroundProcessor>();
			...
			{
				c.SwaggerDoc("v1", new Info {Title = "Timebattle API", Version = "v1"});
		...
		public void Configure(IApplicationBuilder app, IHostingEnvironment env)
		{
			...
			app.UseSwaggerUI(c => { c.SwaggerEndpoint("/swagger/v1/swagger.json", "Timebattle API V1"); });

			app.UseHttpsRedirection();
			app.UseSignalR(routes =>
			{
				routes.MapHub<PlayerHub>("/PlayerHub");
				routes.MapHub<SectorHub>("/SectorHub");
