...
namespace Grains.Utility
{
	...
	public class GrainObserverManager<T> : IEnumerable<T> where T : IAddressable
	{
		...
		/// </summary>
		public Int32 Count => observers.Count;
		...
		/// </returns>
		public async Task Notify(Func<T, Task> notification, Func<T, Boolean> predicate = null)
		{
			var now     = GetDateTime();
			...
			foreach (var observer in observers)
			{
				...
				//if (predicate != null && !predicate(observer.Key)) continue;
		...
		/// </param>
		/// <param name="predicate">The predicate used to select observers to notify.</param>
		public void Notify(Action<T> notification, Func<T, Boolean> predicate = null)
		{
			var now     = GetDateTime();
			var defunct = default(List<T>);
			foreach (var observer in observers)
